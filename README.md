...
Astro-Note AI is a desktop app that turns research papers into a local LLM-powered Wiki.
...
When the number of papers grows, the real problem is not only summarization. It is reuse.
PDFs stay isolated as files, notes are tied to the moment they were written, and title or
keyword search is often not enough when you return to an idea weeks later. An LLM Wiki turns
papers into a research knowledge base that can be searched, linked, questioned, revised, and
exported.

The main benefits are:

- Convert paper PDFs into Wiki pages, Korean summaries, topic tags, and graph relationships.
- Connect methods, objects, datasets, and research themes across multiple papers.
- Ask research questions in Chat and retrieve answers grounded in the local Wiki and paper text.
- Save useful answers as Wiki update proposals, making reading, questioning, and note-taking one workflow.
- Export the Wiki as an Obsidian-ready Markdown vault ZIP.
- Keep API keys and personal settings on the user's machine; uploaded papers and generated data are excluded from Git by default.

### Key Features

- Upload multiple PDF papers or Markdown documents
- Automatically generate Wiki pages from paper text
- Generate and regenerate Korean summaries
- Explore PDF, Wiki, Chat, and Graph views in one app
- Discover papers through ADS/arXiv/local search and build Wiki pages from selected results
- Support Ollama, OpenAI API, Gemini, Claude, OpenRouter, Groq, DeepSeek, xAI, Mistral, and OpenAI-compatible APIs
- Render TeX equations with MathJax
- Save Chat Q&A and create Wiki update proposals
- Export an Obsidian vault ZIP
- Store API keys and personal settings locally
...
- Refreshes the research note state after uploads or Wiki generation.
...
- `Work Prompt` lets you inspect, edit, save, or reset the prompt used during upload processing.
...
- Inspect title, arXiv ID, categories, topics, relevance score, and classification rationale.
- Read papers in the built-in PDF viewer with page navigation, zoom, and fit-to-width.
- Use `Info` to load the Korean summary, classification, abstract, and generated Wiki preview.
- Regenerate the Korean summary when needed.
- Use `Wiki` to open the paper Wiki page and `Chat` to ask questions with that paper as context.
- Delete unwanted papers together with their PDF, extracted text, summaries, graph links, and Wiki references.
...
- Render TeX equations with MathJax.
- Review Chat-generated Wiki update proposals and apply them to target pages with `Apply Proposal`.
- Delete a Paper Wiki and its related PDF, extracted text, summaries, graph edges, and Wiki references.
- Export the current Wiki as an Obsidian vault ZIP with `Choose Folder` and `Export ZIP`.
...
- The Discover Papers graph in Upload shows candidate-paper similarity, while the Graph menu shows the local Wiki network.

#### Settings

Settings manages LLM providers and model connections.

- Choose Ollama or an API provider.
- Supports OpenAI API, Google Gemini, Anthropic Claude, OpenRouter, Groq, DeepSeek, xAI, Mistral, and custom OpenAI-compatible endpoints.
- Configure API base URL, API key, NASA ADS API key, and Ollama base URL.
- Select or manually enter Chat and Retrieval models.
- Tests the LLM connection before saving settings.
- Stores settings in the OS-specific user settings directory, outside the Git repository.

#### Review / Runs

These internal management screens are included even though they are not part of the main tab bar.

- Review Queue is used to inspect and apply Wiki update proposals created from Chat.
- Wiki Lint checks Wiki documents for numeric, evidence, and formatting issues.
- Runs is an advanced tool for running agent jobs such as classify, extract, ingest, digest, newsletter, curate, graph, semantic wiki, QMD index, Korean summaries, and lint.
...
Choose an LLM provider in `Settings`.
...
```text
_internal/app/config/   App configuration and prompts
_internal/app/scripts/  Runtime and processing scripts
_internal/app/src/      Python modules
_internal/app/ui/       Frontend files
RUNTIME_REQUIREMENTS.md Linux runtime requirements
```

Windows/macOS package files are managed with Git LFS. PyInstaller runtime folders, uploaded PDFs,
generated Wiki/Graph/cache/log files, and local user data are not committed.

### Data Locations

Generated app data is stored under these paths:

```text
data/raw/papers/       Uploaded PDFs
data/markdown/         Extracted Markdown
data/text/             Extracted text
data/summaries/        Korean summaries
wiki/papers/           Paper Wiki pages
wiki/document/         Markdown document Wiki pages
graphify-out/          Graph viewer output
```

User-specific LLM settings are stored outside the package:
...
Before publishing a public repository, check that test PDFs, generated summaries/Wiki pages,
API keys, and personal research data are not included. This repository configuration excludes
that generated data by default.
